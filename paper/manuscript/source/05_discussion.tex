...
advantage over the flattened Random Forest reference (RF-260). This result
...
\smallskip
The loss-component ablation clarifies how this stability arises. Teacher
initialisation is the load-bearing element: randomly initialised students
collapse even under the full anchoring objective, so the anchoring terms act
only once the student already occupies the teacher's region of parameter
space. Among teacher-initialised configurations the predictive metrics are
statistically indistinguishable, which confirms that the anchoring terms
preserve teacher-level prediction rather than improving it, while the
route-consistency term measurably tightens routing and tree-contribution
fidelity. The individual contributions of teacher initialisation, logit
anchoring, and route anchoring are therefore separated rather than evaluated
only in combination.
...
the disclosed explanation-only post-hoc test analysis provides corroborating
explanation evidence rather than primary predictive evidence.
