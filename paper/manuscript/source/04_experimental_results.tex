...
\smallskip
Figures~\ref{fig:r52_same_information_fig}
and~\ref{fig:r52_temporal_value_fig} present the same-information and
temporal-value results, respectively.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.84\textwidth,keepaspectratio]
{figures/fig_same_information_comparison.png}
\caption{Same-information CERT r5.2 comparison. Every model received the
same 20-day by 13-feature input; the sequence models kept the
\(20\times13\) structure, while the others received the same 260 values as
a flattened vector.}
\label{fig:r52_same_information_fig}
\end{figure*}
...
separately as a sensitivity analysis. A disclosed post-hoc CERT r5.2 test
analysis reproduced the centred pattern and is used only as corroborating
evidence.
...
Figure~\ref{fig:odst_calibration} shows the calibration change for the Brier
...
keepaspectratio
]{figures/fig_odst_calibration_compact.png}
\caption{Seed-42 ODST calibration before and after grouped out-of-fold
temperature scaling. Lower values indicate better probability
calibration.}
\label{fig:odst_calibration}
\end{figure}
