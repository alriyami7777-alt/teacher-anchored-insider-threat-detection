...
No final-student test result is reported in this paper.
...
temporal attention into the context vector \(\mathbf c_i\), and classified
...
\(\mathbf c_i\in\mathbb R^{128}\) is the context vector passed to the tree
head. Intuitively, \(\alpha_{i,t}\) is how much day \(t\) contributes to the
decision, and \(\mathbf c_i\) is the attention-weighted summary of the whole
window.

\subsubsection{Oblivious differentiable sparsemax tree (ODST) head}
\label{subsubsec:odst}
The context vector \(\mathbf c_i\) is classified by a differentiable
...
\mathbf q_i^{(2)}
=
\big[\,\mathbf c_i\;;\;\mathbf O_i^{(1)}\,\big]
\in\mathbb R^{128+8}=\mathbb R^{136}.
\label{eq:stack}
\end{equation}
For uniformity we write \(\mathbf q_i^{(1)}=\mathbf c_i\) for the first
...
\smallskip
To isolate the contribution of each element of this objective, a
loss-component ablation was conducted under the same CERT r5.2
train/validation protocol, preprocessing, seeds (42, 52, and 62), optimiser
settings, epoch limit, early-stopping rule, and threshold selection as the
final students. Six configurations were compared against the frozen teacher:
a randomly initialised student trained with the classification loss only
(C1); a teacher-initialised student trained with the classification loss
only (C2), which isolates the effect of initialisation; teacher-initialised
students adding the logit term alone
(C3, \(\lambda_{\mathrm{logit}}=0.5,\ \lambda_{\mathrm{route}}=0\)) or the
route term alone
(C4, \(\lambda_{\mathrm{logit}}=0,\ \lambda_{\mathrm{route}}=0.5\)); the full
teacher-anchored objective
(C5, \(\lambda_{\mathrm{logit}}=\lambda_{\mathrm{route}}=0.5\)), which is the
configuration trained for the final students; and a randomly initialised
student trained with the full objective (C6). Configuration~C5 was
additionally re-evaluated at
\(\lambda_{\mathrm{logit}}=\lambda_{\mathrm{route}}\in\{0.25,0.75\}\) as a
pre-specified sensitivity check around the locked value \(0.5\); these weights
were not selected retrospectively. The saved final student supported
teacher-independent inference.

\smallskip
Each configuration was scored for predictive retention---PR-AUC, F1, and
their student-minus-teacher differences---and for four routing- and
explanation-fidelity measures computed on the r5.2 validation partition
against the corresponding frozen teacher. The route mean absolute error
(route MAE) is the mean absolute difference between the student and teacher
right-branch probabilities of Equation~\eqref{eq:split}, averaged over all
\(B\times16\times4\) split entries, so smaller values indicate closer
routing. Hard-route agreement is the fraction of those split entries on which
the student and teacher send the sample to the same branch, that is, agree on
whether \(p_{i,m,k}>0.5\). The unused-leaf fraction is the percentage of the
\(2^{D}=16\) leaves per tree whose soft membership
\(\mu_{i,m,\mathbf e}\) of Equation~\eqref{eq:leaf_prob} never exceeds a small
activation threshold on the validation set, serving as a route-concentration
indicator. The tree-contribution Spearman correlation is the mean, over
validation sequences, of the Spearman rank correlation between the teacher and
student reference-centred tree-contribution vectors \(c_{i,m}\) of
Equation~\eqref{eq:tree_contribution}, and measures whether the two models
rank the sixteen trees' contributions consistently. The results are reported
in Section~\ref{subsec:loss_ablation}.
