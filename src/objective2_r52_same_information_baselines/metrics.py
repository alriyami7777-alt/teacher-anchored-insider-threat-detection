...
def choose_threshold(y_val: np.ndarray, p_val: np.ndarray) -> tuple[float, float]:
    """Locked r4.2/r5.2 procedure: max validation F1 on linspace + probability quantiles."""
...
def summarise_seeds(rows: list[dict], metric_keys: list[str]) -> dict:
    out: dict = {}
    for k in metric_keys:
        vals = [float(r[k]) for r in rows if r.get(k) is not None and r.get(k) == r.get(k)]
        if not vals:
