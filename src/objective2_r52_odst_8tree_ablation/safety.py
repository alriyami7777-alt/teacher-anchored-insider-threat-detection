"""Safety for 8-tree ODST ablation (validation-only; no test)."""
...
def assert_path_allowed_for_read(path: Path | str, *, context: str = "") -> Path:
    ...
    if "test.npz" in s or s.endswith("/test") or "/test/" in s:
...
def assert_output_namespace(path: Path | str) -> Path:
    p = Path(path).resolve()
    if "r52_odst_8tree_ablation_v1" not in _norm(p):
        raise ProtectedDataAccessError(f"Write outside namespace: {p}")
    return p


def refuse_test_loader() -> None:
    raise ProtectedDataAccessError("8-tree ablation forbids test loader construction")
...
class OpenedFilesRegister:
    ...
    def record(self, path: Path, purpose: str) -> Path:
        assert_path_allowed_for_read(path, context=purpose)
        self.rows.append({"path": str(Path(path).resolve()), "purpose": purpose})
        return Path(path)
